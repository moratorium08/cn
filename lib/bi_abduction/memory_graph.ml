...
let empty = { adj = Int64Map.empty; info = Int64Map.empty }
...
let add_node addr node_info g =
  ...
  let adj =
    if Int64Map.mem addr g.adj then
      g.adj
    else
      Int64Map.add addr [] g.adj
  in
  { adj; info }


let add_edge src dst edge g =
  let existing = match Int64Map.find_opt src g.adj with Some l -> l | None -> [] in
...
let build
  ...
  let anchor_addrs =
    StdList.fold_left
      (fun acc (v : Data_point.var_binding) -> Int64Set.add v.value acc)
      Int64Set.empty
      pre_vars
  in
  Pp.debug 4 (lazy (Pp.item "graph: anchors" (Pp.int (Int64Set.cardinal anchor_addrs))));
  Pp.debug
    4
    (lazy (Pp.item "graph: missing bytes" (Pp.int (Int64Set.cardinal missing_set))));
  let g = ref empty in
  ...
  Int64Set.iter
    (fun addr ->
       ...
       := add_node
            addr
            { is_anchor = Int64Set.mem addr anchor_addrs; is_missing = true }
            !g)
    missing_set;
  ...
  Int64Set.iter
    (fun addr ->
       Pp.debug
         5
         (lazy (Pp.item "graph: anchor" (Pp.string (Printf.sprintf "0x%Lx" addr))));
       g
       := add_node
            addr
            { is_anchor = true; is_missing = Int64Set.mem addr missing_set }
            !g;
       Queue.add addr worklist)
    anchor_addrs;
  while (not (Queue.is_empty worklist)) && Int64Map.cardinal !g.info < max_nodes do
    let base = Queue.pop worklist in
    if not (Int64Set.mem base !visited) then (
                  ...
                  | Some target when Int64.compare target 0L <> 0 ->
                    ...
                    if not (Int64Map.mem target !g.info) then
                      ...
                      := add_node
                           target
                           { is_anchor = false;
                             is_missing = Int64Set.mem target missing_set
                           }
                           !g;
  ...
  Pp.debug
    ...
    (lazy
      (Pp.string
         (Printf.sprintf
            "graph: BFS done, %d nodes, %d visited"
            total_nodes
            total_visited)));
  !g
...
let reachable_from (g : t) (start : int64) : Int64Set.t =
  ...
  let rec walk addr =
    if Int64Set.mem addr !visited then
      ...
      | None -> ()
      | Some edges -> StdList.iter (fun (dst, _) -> walk dst) edges)
  in
  walk start;
  !visited
...
let anchors (g : t) : Int64Set.t =
  Int64Map.fold
    (fun addr info acc -> if info.is_anchor then Int64Set.add addr acc else acc)
    g.info
    Int64Set.empty
...
let missing (g : t) : Int64Set.t =
  Int64Map.fold
    (fun addr info acc -> if info.is_missing then Int64Set.add addr acc else acc)
    g.info
    Int64Set.empty


(** Compute the total size of each struct layout (max of offset + size across fields). *)
let struct_total_sizes (struct_layouts : (Id.t * int * int) list Sym.Map.t) : int list =
  Sym.Map.fold
    (fun _tag fields acc ->
       let total =
         StdList.fold_left (fun mx (_fid, off, sz) -> max mx (off + sz)) 0 fields
       in
       total :: acc)
    struct_layouts
    []
...
let reachable_struct_bytes
  ...
  Int64Set.fold
    (fun addr acc ->
       ...
       let has_outgoing =
         match Int64Map.find_opt addr g.adj with Some (_ :: _) -> true | _ -> false
       in
